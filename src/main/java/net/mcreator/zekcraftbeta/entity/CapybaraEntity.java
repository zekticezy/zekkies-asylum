...
public class CapybaraEntity extends TamableAnimal {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ZekcraftBetaModEntities.CAPYBARA.get(), 20, 4, 4));
	}

	public CapybaraEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ZekcraftBetaModEntities.CAPYBARA.get(), world);
	}
	...
	public SoundEvent getAmbientSound() {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("zekcraft_beta:capybark"));
	}
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		CapybaraEntity retval = ZekcraftBetaModEntities.CAPYBARA.get().create(serverWorld);
	...
	public static void init() {
		SpawnPlacements.register(ZekcraftBetaModEntities.CAPYBARA.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
